...
$this->import('
    agent-data-1
    agent-federative-entities-list
...
$federativeEntities = UserAccessService::isSaasSuperAdmin()
    ? (new FederativeEntityAdminService($app))->getAgentFederativeEntitiesData($entity)
    : [];
...
<div class="main-app single-1">
    ...
    <mc-tabs class="tabs" sync-hash>
        ...
        <?php if ($federativeEntities): ?>
            <mc-tab label="<?= i::_e('Entes') ?>" slug="federative-entities">
                <div class="tabs__info">
                    <mc-container>
                        <main>
                            <div class="grid-1 col-12">
                                <agent-federative-entities-list
                                    class="col-12"
                                    title="<?php i::esc_attr_e('Entes Federados');?>"
                                    :entities='<?= json_encode($federativeEntities, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>'>
                                </agent-federative-entities-list>
                            </div>
                        </main>
                        <aside></aside>
